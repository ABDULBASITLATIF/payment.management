...
sap.ui.define([
    ...
    return Controller.extend("zfi.payment.management.controller.PostOutgoing", {
        ...
        _populateFormFields: function (oHead) {
            ...
            this.getView().getModel("pageModel").setProperty("/fiscYear", oHead.fiscYear || "");
        ...
        _updateTableTitles: function() {
            ...
            // ── Total Payment Selected: sum of assignAmt where itemType === "P" ────
            const fTotalPaymentSelected = aItemsToBeCleared.reduce(function(fSum, oItem) {
                return oItem.itemType === "P" ? fSum + (parseFloat(oItem.assignAmt) || 0) : fSum;
            }, 0);

            // ── Total Invoice Sum: sum of assignAmt where itemType === "I" ─────────
            const fTotalInvoiceSum = aItemsToBeCleared.reduce(function(fSum, oItem) {
                return oItem.itemType === "I" ? fSum + (parseFloat(oItem.assignAmt) || 0) : fSum;
            }, 0);

            const oPayAmntInput = this.byId("_IDGenInput3");
            const fPayAmnt = oPayAmntInput ? (parseFloat(oPayAmntInput.getValue()) || 0) : 0;

            // ── Balance = Pay Amount + Total Payment Selected - Total Invoice Sum ──
            const fBalance = fPayAmnt + fTotalPaymentSelected - fTotalInvoiceSum;

            const oPaymentSelectedInput = this.byId("_IDGenInputPaySel");
            if (oPaymentSelectedInput) {
                oPaymentSelectedInput.setValue(fTotalPaymentSelected.toFixed(3));
            }
        ...
        onClearItem: function(oEvt) {
            ...
            // ── Initialize reference amounts + editable AssignAmt ──────────────────
            oItem.refAmntLC = oItem.amntLC || "0.000";
            oItem.refAmntDC = oItem.amntDC || "0.000";
            oItem.assignAmt = oItem.amntDC || "0.000";
        ...
        onAssignAmtChange: function(oEvt) {
            const oInput = oEvt.getSource();
            const oContext = oInput.getBindingContext("openItems");
            if (oContext) {
                this.getView().getModel("openItems").setProperty("assignAmt", oInput.getValue(), oContext);
            }
            this._updateTableTitles();
        },

        formatItemType: function(sValue) {
            if (sValue === "P") { return "Payment"; }
            if (sValue === "I") { return "Invoice"; }
            return sValue || "";
        },
            ...
            if (aItemsToBeCleared.length === 0) {
                ...
                if (Math.abs(fBalance) >= 0.001) {
                    MessageBox.error(
                        ...
                        "Total Payment Selected: "  + fTotalPaymentSelected.toFixed(3) + "\n" +
            ...
            const fnSubmit = function(sDraftId) {
                ...
                oDataModel.create("/head", { draftId: sDraftId, action: "S" }, {
                    success: function(oData) {
                        ...
                        if (oData && oData.procStat === "E") {
                            MessageBox.error(oData.msg || "Failed to submit payment", {
                                onClose: function() {
                                    const oRouter = that.getOwnerComponent().getRouter();
                                    oRouter.navTo("RouteNewDoc");
                                }
